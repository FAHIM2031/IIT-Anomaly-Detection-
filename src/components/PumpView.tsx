    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" id="pump-view-container">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-1.5 border-b border-slate-50 pb-3">
            <Cpu className="h-4.5 w-4.5 text-indigo-500" />
            Sprinkler Valve Relay Panel
          </h3>

          <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-slate-50/50 rounded-2xl p-6 border border-slate-100">
            {/* Status Indicator */}
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Actuator Relay Status</p>
                <h2 className={`text-3xl font-extrabold font-mono mt-1 ${isPumpOn ? "text-blue-600" : "text-slate-700"}`}>
                ...
                <div className="flex items-center gap-1.5 text-xs font-medium text-slate-500 mt-1">
                  ...
                  {isPumpOn ? (
                    <span>
                      Active: Sprinkler valves open (Source: <span className="font-bold text-blue-700 underline">{pumpTriggeredBy || "System"}</span>)
                    </span>
                  ) : (
            ...
            <div className="flex flex-col gap-2 w-full md:w-auto">
              {manualPumpOverride && (
                <button
                  ...
                  disabled={isLoading}
                  className="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-2 px-4 rounded-xl text-xs transition duration-150 cursor-pointer flex items-center justify-center gap-1 border border-slate-200"
          ...
          <div className="space-y-4" id="interlocking-steps">
            ...
            <div className="flex gap-4">
              <div className="flex flex-col items-center">
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${anyFlame ? "bg-red-100 text-red-600" : "bg-indigo-50 text-indigo-500"}`}>
                  1
                </span>
                <div className="w-0.5 h-10 bg-slate-100" />
              </div>
              <div>
                <h4 className="font-bold text-slate-700 text-xs">Flame Sensor Trigger Detection</h4>
                ...
                <div className="mt-2 flex items-center gap-1.5">
                  <span className={`text-[10px] px-2 py-0.5 rounded font-bold border ${anyFlame ? "bg-red-50 text-red-700 border-red-100" : "bg-emerald-50 text-emerald-700 border-emerald-100"}`}>
            ...
            <div className="flex gap-4">
              <div className="flex flex-col items-center">
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${anyFlame ? "bg-red-100 text-red-600 animate-pulse" : "bg-slate-50 text-slate-400"}`}>
                  2
                </span>
                <div className="w-0.5 h-10 bg-slate-100" />
              </div>
              <div>
                <h4 className="font-bold text-slate-700 text-xs">Solenoid & Pump Relay Actuation</h4>
                <p className="text-xs text-slate-400 mt-1">
                ...
                <div className="mt-2 flex items-center gap-1.5">
                  <span className={`text-[10px] px-2 py-0.5 rounded font-bold border ${isPumpOn ? "bg-blue-50 text-blue-700 border-blue-100" : "bg-slate-50 text-slate-500 border-slate-200"}`}>
            ...
            <div className="flex gap-4">
              <div className="flex flex-col items-center">
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${isPumpOn ? "bg-blue-100 text-blue-600 animate-bounce" : "bg-slate-50 text-slate-400"}`}>
                  3
                </span>
              </div>
              <div>
                <h4 className="font-bold text-slate-700 text-xs">Automatic Extinguishing Containment</h4>
                <p className="text-xs text-slate-400 mt-1">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 h-fit" id="pump-override-card">
        ...
        {/* Mode Indicator */}
        <div className="p-4 rounded-xl mb-4 border flex items-center justify-between bg-slate-50/50 border-slate-100">
          <div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Control Mode</p>
            <h4 className="text-xs font-bold text-slate-700 mt-0.5">
              {manualPumpOverride ? "MANUAL OVERRIDE ACTIVE" : "AUTOMATED CASCADE MODE"}
            </h4>
          </div>
          <span className={`px-2 py-0.5 rounded text-[9px] font-bold ${manualPumpOverride ? "bg-amber-100 text-amber-800" : "bg-indigo-100 text-indigo-800"}`}>
        ...
        <div className="mt-5 p-3.5 bg-amber-50 rounded-xl border border-amber-100/50 flex items-start gap-2 text-[11px] text-amber-700">
